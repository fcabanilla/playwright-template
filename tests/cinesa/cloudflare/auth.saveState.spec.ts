...
test('login manual y guarda estado (auto-close)', async ({ browser }) => {
  ...
  await page.goto(url);
  console.log(
    'Logueate manualmente y pasa Cloudflare. El script detectará automáticamente cuando entres a la página principal.'
  );
  ...
  try {
    ...
    console.log(
      '¡Página principal detectada! Guardando estado y cerrando navegador...'
    );
    ...
    console.log(
      'No se detectó el navbar en 60 minutos. Revisa el login o Cloudflare.'
    );
  }
});
