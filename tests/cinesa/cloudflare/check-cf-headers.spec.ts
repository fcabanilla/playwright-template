...
type EnvName = 'production' | 'preprod' | 'lab' | 'staging' | 'development';
...
test('check Cinesa URLs with CF-Access headers', async ({ browser }) => {
  const envs: EnvName[] = ['production', 'preprod', 'lab', 'staging', 'development'];
  ...
  for (const env of envs) {
    ...
    try {
      ...
      console.log(`Found Cloudflare credentials for env=${env}, keyUsed=${creds.envKeyUsed}`);
    } catch (err: any) {
      console.log(`No Cloudflare credentials found for env=${env}: ${err?.message || err}`);
      creds = undefined;
    }
    ...
    if (creds) {
      ...
      console.log('Setting extra HTTP headers:', Object.keys(headers).join(', '));
    ...
    try {
      ...
      finalUrl = page.url();
      status = resp?.status();
      console.log(`Navigation finished. status=${status}, finalUrl=${finalUrl}`);
    } catch (err: any) {
      console.log(`Navigation error for ${env}:`, err.message || err);
      // try to capture whatever is available
      try { finalUrl = page.url(); console.log('page.url() after error:', finalUrl); } catch (e) { /* ignore */ }
    }

    // Check for Cloudflare patterns in finalUrl
    const isCloudflare = /cloudflare|cloudflareaccess|cdn-cgi\/access/i.test(finalUrl);
