...
import { filmsSelectors, FilmsSelectors } from './films.selectors';
...
export class Films {
  ...
  async getFilmTitles(): Promise<string[]> {
    return await allure.test.step('Getting list of film titles', async () => {
      ...
      try {
        // First try to get titles directly using the specific UCI selector
        ...
        if (titleCount > 0) {
          console.log(`🎬 Found ${titleCount} films with UCI selector`);
        ...
        // Fallback: try with program-list-element approach
        const filmCards = await this.webActions.getElementCount(
          this.selectors.filmCard
        );
        console.log(`🎬 Fallback: Found ${filmCards} film cards`);

        for (let i = 0; i < filmCards; i++) {
          const titleSelector = `${this.selectors.filmCard}:nth-child(${i + 1}) ${this.selectors.filmTitle}`;
          try {
  ...
  async selectFilmByTitle(filmTitle: string): Promise<void> {
    await allure.test.step(`Selecting film: ${filmTitle}`, async () => {
      const filmSelector = `${this.selectors.filmCard}:has-text("${filmTitle}")`;
  ...
  async selectFirstFilm(): Promise<string> {
    return await allure.test.step(
      ...
      async () => {
        const firstFilmSelector = `${this.selectors.filmCard}:first-child`;
  ...
  async searchFilms(searchTerm: string): Promise<void> {
    await allure.test.step(`Searching for films: ${searchTerm}`, async () => {
      ...
      await this.webActions.wait(1000); // Wait for search results to load
    });
  }
  ...
  async clickBookNow(filmTitle?: string): Promise<void> {
    await allure.test.step('Clicking book now button', async () => {
      if (filmTitle) {
        const filmSelector = `${this.selectors.filmCard}:has-text("${filmTitle}") ${this.selectors.bookNowButton}`;
        await this.webActions.click(filmSelector);
      } else {
  ...
  async isFilmsPageLoaded(): Promise<boolean> {
    return await allure.test.step(
      ...
      async () => {
        try {
          // First check if we have UCI-specific film elements
          const uciFilmCount = await this.webActions.page
            .locator('h2[data-v-64b23b32]')
            .count();
          if (uciFilmCount > 0) {
            console.log(`✅ UCI films page loaded with ${uciFilmCount} films`);
            return true;
          }
